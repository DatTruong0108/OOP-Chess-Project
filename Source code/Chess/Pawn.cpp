...
vector<ChessMove> Pawn::computePossbibleMoves(const vector<vector<Cell>>& cells) {
    ...
    if (pos.y - 1 >= 0) {
        Cell cell = cells[pos.x][pos.y - 1];
        if ((pos.x == 3 && this->_pieceColor == PieceColor::WHITE) || (pos.x == 4 && this->_pieceColor == PieceColor::BLACK)) {
            if (cell._status == CellStatus::OCCUPIED && cell._piece->getPieceColor() != this->_pieceColor) {
                if (cell._piece->getPieceType() == PieceType::PAWN) {
                    auto pawn = dynamic_cast<Pawn*>(cell._piece.get());

                    if (pawn->_hasDoubleMoved == true) {
                        pawnMove._type = MoveType::EN_PASSANT;
                        pawnMove._position = Vector2i(xnew, pos.y - 1);
                        possibleMoves.push_back(pawnMove);
                    }
                }
            }
    ...
    //check for a double forward step, if this is the first move of the pawn,
